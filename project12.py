...
class TwitterClient(object): 
    ...
    def __init__(self): 
        ...
        consumer_key = 'qJ14NxTxxxxxxxxxxxx'
        consumer_secret = 'hJYpqfMe7DtfOxxxxxxxxxxxxxxx'
        access_token = '1046088765993701xxxxxxxxxxxxxxxxxxxxxxx'
        access_token_secret = 'zVWKo6NPDzxxxxxxxxxxxxxxxxxxxxx'
...
class PorterStemmer:
    ...
    def option5b(self, word):
        if self.getM(word) > 1 and self.doubleCons(word) and self.endsWith(word, 'l'):
            word = word[:-1]
        return word

    def stem(self, word):
        word = self.option1a(word)
        word = self.option1b(word)
        word = self.option1c(word)
        word = self.option2(word)
        word = self.option3(word)
        word = self.option4(word)
        word = self.option5a(word)
        word = self.option5b(word)        
        return word
